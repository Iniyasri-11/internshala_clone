...
const jobPath = path.join(__dirname, '..', 'src', 'pages', 'detailjob', '[id]', 'index.tsx');
const internPath = path.join(__dirname, '..', 'src', 'pages', 'detailiternship', '[id]', 'index.tsx');

const jobSource = `import Link from "next/link";
...
const internSource = `import Link from "next/link";
import { useRouter } from "next/router";
import React, { useEffect, useState } from "react";
...
fs.writeFileSync(jobPath, jobSource, 'utf8');
fs.writeFileSync(internPath, internSource, 'utf8');
console.log('Rewrote', jobPath);
console.log('Rewrote', internPath);
