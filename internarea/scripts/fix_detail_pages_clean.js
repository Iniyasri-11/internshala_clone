      ...
      <div className="grid grid-cols-1 xl:grid-cols-[3fr_2fr] gap-8">
        ...
        <aside className="space-y-6">
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6">
            ...
            <p className="mt-3 text-sm text-gray-500">Please login before applying. Your resume will be submitted with your application.</p>
          </div>
        </aside>
      </div>
...
import { useRouter } from "next/router";
import React, { useEffect, useState } from "react";
import { ArrowUpRight, Calendar, Check, Clock, DollarSign, ExternalLink, MapPin, X } from "lucide-react";
import { api } from "@/utils/api";
import { toast } from "react-toastify";
import { useSelector } from "react-redux";
import { selectuser } from "@/Feature/Userslice";
...
fs.mkdirSync(path.dirname(internshipPath), { recursive: true });
fs.writeFileSync(jobPath, jobContent, "utf8");
fs.writeFileSync(internshipPath, internshipContent, "utf8");
console.log('Wrote detail pages to', jobPath, internshipPath);
